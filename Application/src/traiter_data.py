...
def nommer_colonnes(tables):
    ...
    for i, table in enumerate(tables):
        ...
        nouveau_header = [cell[0].split(" ")[0] for cell in nouvelle_table.iloc[0]]
...
code_obtention_pv = ("adm", "def", "cmp", "vac", "aj", "adj", "cms", "reo", "ajac")
...
def notes_possibles(case):  # Fonctions qui retourne les valeurs pouvant être une note
    liste = []
    for elem in case:
        for splitter in elem.split(" "):
            # attention à true et false
            if isfloat(splitter) and float(splitter) >= 0 and float(splitter) <= 20:
...
def credits_possibles(case):
    ...
    for elem in case:
        if isfloat(elem) and (
            float(elem) == 3
            or float(elem) == 6
            or (float(elem) > 20 and float(elem) % 3 == 0)
        ):
...
def code_obtention(case):  # fonction pour obtenir le code d'obtention d'une case
    for elem in case:
        for splitter in elem.split(" "):
...
def traiter_identification(cell):
    ...
    if not infos_reduites:
        ...
        new_cell.append(
            new_cell[3] == "02a" or new_cell[3] == "02b" or int(new_cell[3]) <= 96
        )
...
def fusionner_liste_df(liste_df):
    # Créé une liste de dataframe, chaque dataframe de cette liste a un ensemble
    # de cours differents. Et chaque dataframe rassemble l'ensemble des
    # étudiants. Une fois ces dataframes créé, on fait une jointure pour avoir
    ...
    for tableau in liste_df_indexed:
        for column in tableau.columns:
            if (
                column not in fused_df.columns
        ...
        for num_etudiant, row in tableau.iterrows():
            if (
                num_etudiant not in fused_df.index
